...
# Push only the named module (annotations). The --exclude-unnamed flag
# skips the proto/examples workspace module, which has no name and must
# not reach BSR.
buf push --exclude-unnamed
```

`buf push` without `--exclude-unnamed` will fail with *"a name must be specified in buf.yaml to push module: path: proto/examples"* — that's expected and intentional; the flag is how we tell buf to publish the one and skip the other.

Annotation schema changes are subject to stricter rules:

- **No tag-number reuse, no removed fields, no renames.** `protomcp.v1` is consumed by downstream generators; breaking it breaks everyone's generated code.
- **`buf breaking`** is run against the BSR copy in CI — if it flags a real break, push the change as a new `v2` package instead.
...
An RPC is exposed as an MCP tool **only** when it carries the `protomcp.v1.tool` method option. Unannotated RPCs never leak. There is no denylist mode and no auto-expose default. The rationale is safety — `protoc-gen-mcp` runs in repos that contain internal-only RPCs we must not accidentally expose to an LLM.
...
### 6. Client-streaming and bidi-streaming are errors, not warnings

An annotated client-streaming or bidi RPC fails generation with a clear error citing the `service.method`. We do not emit a best-effort tool; there is no sensible protojson ↔ MCP mapping for duplex streams.
...
3. Thread the field through `internal/gen/annotations.go` and the tool template.
4. Update the annotation reference table in `README.md`.
5. Add a test in `internal/gen/generator_test.go` with a fixture proto exercising the new field.
